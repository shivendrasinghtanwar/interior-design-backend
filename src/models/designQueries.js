class Queries {
  fetchAssignedClientQuery(data) {
    return `select DATE_format(c.meeting_datetime,'%d %b %y %h:%i %p') AS meetingDateTime,c.id,concat(c.title,' ',c.first_name,' ',c.last_name) AS name, c.email,c.mobile,ad.city,p.id AS projectId,p.scope_of_work from client_assigned ca join client c on c.id = ca.client_id join projects p on p.client_id = c.id join address_details ad on ad.client_id = c.id where c.status=3 and ca.admin_id = ${data.adminId}  ;`;
  }
